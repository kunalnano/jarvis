...
### Added
- Yennefer pet status API at `/api/pet/status`, combining backend, hand-up, model, voice, speech, and pending-action state into a fast read-only contract.
- Compact capsule pet surface in the local web chat, with idle, listening, thinking, speaking, approval-needed, degraded, warning, and offline visual states.

### Changed
- Gated tool approvals now update pet attention state until the user approves or declines.
...
- **Mermaid architecture diagram** - Color-coded flowchart with styled nodes replacing ASCII art
